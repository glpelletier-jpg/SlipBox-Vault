---
doc_type: procedure
purpose: "Vault architecture, plugins, templates, folder structure, and maintenance cadence"
...
**Today's schedule section:** When a new daily note is created, the `### 📅 Today's schedule` section auto-populates via `daily-schedule.js`. It shows:
- Google Calendar personal events (time range + title)
- Argenta work calendar events with a 🏢 prefix
- Todoist tasks due today with a ✅ prefix

Review this section as part of Step 2 — it's your actual day shape before you write your focus intention. The schedule only populates on note *creation*; it does not refresh if you press `Ctrl+D` on an existing note. If it shows a warning about a missing ICS URL, check QuickAdd → Manage Macros → Weekly Calendar → ⚙️ gear icon.
...
**What it is:** Six checkboxes in the daily note — three per active quest. Check them as you complete each habit during the day or at end of day.

**How to do it:**
1. Find the "Quest habits" section
2. **Health Reset Q3** habits:
   - `🏃 Moved for 30+ min`
   - `😴 In bed by target time`
   - `🥗 Avoided main dietary drain`
3. **New Role Integration Q3** habits:
...
### Step 5: Evening — copy focus total to frontmatter (automated)
...
1. At end of day, press `Ctrl+P` → "Update Focus Minutes" → Enter (or assign `Ctrl+Shift+M` in Settings → Hotkeys → search "Templater: Create new note from template" and add a hotkey for the Update Focus Minutes template)
...
| `sleep_hours` | Hours of sleep from last night (e.g., 7.5) | Morning, when you wake up |
...
**The 1,200 min / 20 hr weekly target** is a suggested baseline. You're currently averaging much higher (600+ min/day × 5 days). The point is to track the trend, not hit a specific number.
...
5. Each quest has 4 substantive milestones worth 25 points each (4 × 25 = 100%). The "Final (week 12)" item in the template marks completion of the Month 3 milestone — it is not a fifth independent milestone.
...
3. Open Google Calendar: block time for your 3 priorities. If you don't block it, it won't happen.
...
**How to fill it in:**
1. **Focus time total:** Sum all `focus_minutes:` from each weekly review, or add up all daily notes. Copy the total to `focus_minutes_total:` in Properties. *This is the field the quarterly review Dataview reads to show per-month focus trends — if you skip this step, the quarterly Section 2 table shows dashes for this month.*
...
**Section 2 — Quarter in numbers:** The Dataview table auto-populates all weekly data. Fill in the summary metrics manually (total focus hours, average weekly rating, quests completed, books finished). Copy `focus_minutes_total` and quest counts to frontmatter.
...
4. In the book note's "Linked notes" section, add: `[[06-Zettelkasten/Literature/|Literature folder]]`
...
2. **Rename immediately** to the article title — keep it short and recognizable
3. Fill in the Properties panel:
   - `url:` — the full URL of the article
   - `author:` — author name if known
   - `source:` — publication, website, or origin (e.g., "ISA.org", "Cal Newport blog", "Medium")
...
- **Why this is worth reading** — write one sentence about what you expect to get from it *before* reading. This primes your reading intention.
- **Key ideas** — as you read, write the key ideas in your own words (not copy-paste)
...
**The Articles base view** (`05-Resources/Articles/Articles.md`) has four tabs: To read, Reading, Done, and All articles. Sorted by date saved so the most recent captures are at the top.

**Note on Web Clipper:** The Obsidian Web Clipper browser extension (free, official — install at obsidian.md/clipper) can auto-fill the Article Template from any webpage with one click — title, URL, author, and source are populated automatically. This is the recommended way to save articles quickly without leaving the browser.

---

### Adding a PDF to the vault

See the full procedure in `_attachments/Attachment Catalog.md`. In brief:
...
3. **Create a companion note** in the same subfolder using the Attachment Note Template (`Ctrl+P` → "Templater: Insert Template" → "Attachment Note Template"). Name it identically to the PDF without the extension.
...
6. Copy `time_to_resolve:` to the frontmatter (in minutes) so the aggregate statistics work

The Troubleshooting folder note has live Dataview tables showing: all incidents by date, open incidents, and root cause pattern analysis across all logs.
...
2. Create a new note there — Templater may not apply a specific template, so start with a clear heading and write what you learned
...
**Naming convention for work knowledge notes:** Use descriptive names, not topic names. "Powerflex 753 overcurrent fault during ramp-up" is better than "VFD faults."
...
| **Google Calendar** | Time-bound events; time-blocking; the daily schedule view | Task management, note-taking |

**The key rule:** If something needs to happen by a certain time, it's in Todoist. If it needs to be reflected on or built upon, it's in Obsidian. If it needs a time slot on the calendar, it's in Google Calendar.

---

### The morning sequence (do these in order)

**Step 1 — Google Calendar first (3 min)**
Open Google Calendar before anything else. See the shape of the day: what meetings are fixed? How much unstructured time actually exists? Planning a full task day and then losing it to meetings is the most common morning mistake.

**Step 2 — Todoist Today view (5 min)**
Open `todoist://today`. Given what you just saw in Google Calendar:
...
**Step 4 — Google Calendar time-block (2 min)**
Back to Google Calendar: if any Todoist priorities need protected time, block it now. Not "I'll find time" — block it.
...
| Meeting to schedule | Google Calendar directly |
| Handwritten on reMarkable — work sketch or site note | reMarkable `Work/` folder → auto-syncs as PDF → companion note → link to Work Knowledge |
| Handwritten on reMarkable — idea or brainstorm | reMarkable `Fleeting/` folder → auto-syncs → retype into vault fleeting note (`Ctrl+Shift+F`) |

**The rule:** Anything with a due date and a reminder goes in Todoist. Everything else goes in Obsidian. The reMarkable is a capture surface — not a storage layer; knowledge extracted from it lives in vault notes.
...
**Work calendar (Argenta) setup:** The weekly, next-week, and daily schedule scripts support a separate Argenta work calendar. Work events appear with a 🏢 prefix. To configure:
1. Open Google Calendar in a browser → click the gear ⚙️ → Settings
2. Under "Other calendars" or "My calendars," click the Argenta calendar name
3. Scroll to "Integrate calendar" → copy the **"Secret address in iCal format"** URL
4. In Obsidian: Settings → QuickAdd → Manage Macros → Weekly Calendar → ⚙️ gear icon
5. Paste the URL into the **"Work calendar ICS URL"** field
6. Repeat step 4–5 for the **Next Week Calendar** macro

The `daily-schedule.js` script reads the same setting automatically — no separate configuration needed there.
...
**Layer 1 — Git (version history):** Hourly (if Windows Task Scheduler is configured — see [[_docs/Vault Backup System|Vault Backup System]] for setup), the vault auto-commits to a private GitHub repository. Full history — you can roll back any note to any point in time. If Task Scheduler is not yet configured, run `_scripts/vault-backup.bat` manually.
...
1. Open `C:\Vault\backup-log.txt` — look for recent timestamps (should be within the last hour if the computer has been on and Task Scheduler is configured)
...
| New troubleshooting log | `Ctrl+Shift+T` |
| Update focus_minutes | `Ctrl+Shift+M` *(not assigned by default — assign in Settings → Hotkeys)* |
| Fetch weekly calendar | `Ctrl+Shift+K` |
| Fetch next week calendar | `Ctrl+Shift+J` |
| Open graph view | `Ctrl+G` |
| Extract selection to new note | `Ctrl+Shift+E` |
| Copy Advanced URI | `Ctrl+Shift+U` |
